# data_ingestion_toolbox/census_acs/geography.py
...
def sync_geo_dim(source_year: Optional[int] = None, min_year: int = 2010) -> None:
    ...
    co_df = (
        co.select(
            [
                ...
                .cast(pl.Utf8)
                .str.zfill(5)
                .alias("geoid5"),  # Guarantees length of 5 digits and 0 padded
    ...
    logger.info(
        "geo_dim sync complete: %s rows upserted using Gazetteer year=%s", len(rows), y
    )
