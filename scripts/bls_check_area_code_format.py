"""Operational diagnostic for stored BLS area-code formats."""
...
for row in cur.fetchall():
    print(
        f"  Series: {row[0]}, S={row[1]}, Area={row[2]} (len={len(row[2])}), Measure={row[3]}"
    )
    print(
        f"    Format check: LA{row[1]}{row[2]}{row[3]} = {len('LA' + str(row[1]) + str(row[2]) + str(row[3]))} chars"
    )
