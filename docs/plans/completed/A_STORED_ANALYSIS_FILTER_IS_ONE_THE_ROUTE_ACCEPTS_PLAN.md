...
- **Status:** Accepted 2026-09-14 (Needs review. Investigated and authored 2026-09-13; delivered 2026-09-13 (`4c6fea9`). The sibling of API-091 and API-105 for the analysis kinds; see "What changed" and "Validation".)
...
- `pytest tests/unit/api/test_saved_analysis.py` — **72 passed** (67
  before: +5). **+1 at review on 2026-09-14:** criterion 2's third clause
  ("a stored document from before the change is reported invalid on
  read") had no node. `test_a_stored_analysis_filter_written_before_the_check_is_reported_on_read`
  stores a Census PEP distribution, mutates its filters in storage to the
  `state_fips` the union used to admit, and asserts the read reports it
  invalid with the route's own reason and returns the document verbatim —
  the answer API-112 and API-119 already give for their own stale rows.
