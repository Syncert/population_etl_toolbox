...
- **Next pickup:** none. Review is the remaining step; what this field carried is recorded under the WEB-034 delivery record below.
...
> **Closed 2026-09-12** by `saveDestination` in `apps/web/lib/savedAnalysis.ts`,
> which both screens call — the explorer at `SourceExplorerPage.tsx:1840` and
> the comparison workspace through the same helper — so each saves to the
> account whenever a token is held and says on the button which destination it
> used. Marked here at review on 2026-09-14; closure items 1–3 record the same
...
| Frontend smoke, WEB-027/WEB-033 rerun | `./tests/run.ps1 web-smoke` (2026-09-12, composed origin, image rebuilt) | 10 of 10 passed, exit 0; the two earlier WEB-027 failures were a stale API image, not a client defect |
...
2. ~~Script `'unsafe-inline'` in the CSP.~~ **Closed 2026-09-12.** The
   policy moved into `middleware.ts` with a per-request nonce and
   `'strict-dynamic'`; `'unsafe-inline'` is gone from `script-src`. Every
   route is now dynamically rendered — the trade named here, taken on
   purpose (WEB-032). Worth recording: the first cut left the routes static,
   and the browser suite stayed green under `next dev` while the production
   build served every script tag without a nonce against a policy demanding
