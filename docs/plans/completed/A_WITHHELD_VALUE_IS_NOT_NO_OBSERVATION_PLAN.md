...
- **Status:** Accepted 2026-09-14 (Needs review. Investigated, authored and implemented 2026-09-13. **Found by asking the API-127 question of the map instead of the chart.**)
...
- `comparison.ts`'s synthetic status is now `"not on both sides"`, the phrase
  ...
  **Superseded by WEB-079** (recorded 2026-09-14 at review): one phrase named
  the wrong case for the case that actually happens — `/comparison` joins on
  geography, so every row in the answer *is* on both sides — and it is now the
  row's own reason (`apps/web/lib/comparison.ts:691-710`,
  `tests/frontend/unit/comparison.test.js:641-648`). The shape this plan
  delivered is unchanged; the wording inside it is WEB-079's.
...
395 frontend unit tests and 94 browser specs passed when this plan landed.
Both tiers have grown since; at review on 2026-09-14 the head is 527 web
unit tests and 114 browser specs, all passing.
...
## A defect this plan introduced, found and fixed at review (2026-09-14)

The "nothing to scale" branch replaced an unconditional
`["literal", CHOROPLETH_FALLBACK_COLOR]` with a `match` over the withheld
keys. `withheldMatchPairs` returns `[]` when nothing was withheld, so that
branch could emit a three-argument `match` — an expression MapLibre refuses
outright (`Expected at least 4 arguments`), which means the layer is not
added and the map draws nothing rather than drawing the fallback colour.

It is reachable whenever a page has no number to scale *and* nothing
withheld: a national-grain row whose `geo_id` resolves to no join value
against the tile key is the ordinary way in. Reproduced by calling the
builder directly:

```text
buildChoroplethModel([{geo_id:"US",value:null},{geo_id:"US",value:""}], "geoid")
-> ["match",["to-string",["get","geoid"]],"#9fb0ba"]
```

`apps/web/lib/explorerViewModel.ts` now returns the constant where there is
no label to match on, and keeps the `match` where there is at least one.
`tests/frontend/unit/explorer-contracts.test.js` gains a node for that page,
which fails on the unconditional `match`, and the existing withheld node now
also asserts its `match` is well-formed.
