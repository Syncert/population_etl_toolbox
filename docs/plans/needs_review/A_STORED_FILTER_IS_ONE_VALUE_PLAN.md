...
declared filters plus the two analysis-universal ones. It passes today — the
two sets already agree — but `_require_declared_filters` `continue`s past a
...
`filters` maps a name to one value, which shapes are refused, and that
`state_fips: 6` and `state_fips: "06"` are both accepted because
`?state_fips=6` is.
