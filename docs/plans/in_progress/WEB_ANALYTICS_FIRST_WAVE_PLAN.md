...
- **Status:** Claimed; first pass complete across WEB-001 through WEB-009, and WEB-007's articles remainder closed on 2026-09-12. Still held in `in_progress/` rather than moved to `needs_review/`: every named remainder is closed (items 1–3, 2026-09-12); the one open question from the live-stack smoke tier (see the WEB-033 validation) was answered on 2026-09-12 — a stale API image, not a client defect — so what remains before `needs_review/` is a rerun of this plan's own verify commands
...
- **Next pickup:** rerun this plan's own verify commands (`./tests/run.ps1 web-unit`, `web-browser`, `web-build`) and move this plan to `needs_review/`. The open question this plan was held for is **answered, 2026-09-12**: the two WEB-027 live-stack smoke failures were neither development-stack state nor a client defect. They were a stale `population-etl-api:smoke` image — Compose reuses an image by name, and the four-day-old one still served ACS observations under the pre-ARC-005 `ACS:` lineage identity, so the catalog's `CENSUS_ACS:` codes matched no row. Rebuilt, the tier passes 10 of 10 and exits 0. `tests/run.ps1 web-smoke` now composes the tier's own origin and builds the image it grades; see [`SMOKE_TIER_PROBE_BODIES_PLAN.md`](../needs_review/SMOKE_TIER_PROBE_BODIES_PLAN.md). Items 1–3 under "Remaining before this plan is done" were closed on 2026-09-12 (WEB-031, WEB-032, WEB-033). Item 1, account persistence for evidence packets, was closed on 2026-09-12 by ADR-0004 and its plan. Every phase WEB-001 through WEB-009 has a first pass with inspectable evidence; `docs/reference/WEB_FIRST_WAVE_HANDOFF.md` collects the follow-ons.
