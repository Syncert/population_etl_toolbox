...
- **Next pickup:** Execute DQ-004: extend the reference reconciliation pattern (quality/reconciliation.py, CDC wiring) to source-specific coverage and validity executors for ACS, BLS, FRED, PEP, FBI UCR, and USDA NASS plus the shared reference, with complete and intentionally incomplete fixtures per source.
- **Completed evidence:**
