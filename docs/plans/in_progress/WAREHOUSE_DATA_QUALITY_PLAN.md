...
- **Next pickup:** Decide the superseded-run ledger semantics recorded below (aborted runs finalize their control rows vs. rules assessing only the latest run per partition), then close the two recorded DQ-004/DQ-006 residuals — (1) fixture-driven fact-level defect injections for PEP and FBI UCR (their invariants are DDL-enforced and executor-verified today, with injection coverage at the revision/ledger/control layers); (2) restrict plausibility baselines to quality-certified partitions now that `certify_release` exists (baselines currently use the full retained published history, documented in DATA_QUALITY_OPERATIONS.md). Then re-verify the completion gate and hand the plan to `needs_review/`. All required CI (dag-parse, postgres-integration, etl-unit, coverage, lint) re-verifies on the eventual PR.
- **Known residual (ledger semantics for superseded runs, 2026-08-31):** The slice-ledger rules count every non-terminal row as missing work, including rows from run attempts that were aborted and superseded by a later successful run (observed live: stale `over_limit`/`preflighted` NASS slice rows and failed `control.ingestion_run` rows from the 1990 backfill iterations keep the assessment red even though the final release captured and published the full registered window). Design call pending: either aborted/superseded runs must finalize their control rows to a terminal state, or the ledger rules must assess only the latest run per partition. Until decided, a red daily assessment after any manually re-driven backfill needs operator interpretation.
