...
| API models and query behavior | `api-unit` / `API unit tests (Python 3.11)` | Required PR/push | API, models, SQL query builders |
...
| Browser consumer contract | `frontend` / `Frontend lint, unit, build, and browser` | Required PR/push | web, API models, gold contracts, semantic migrations |
...
| Data-product E2E coverage (E2E-008–E2E-013) | `e2e-performance` / `Bounded E2E and performance evidence`, plus `coverage` for the inventory unit test | Inventory unit test required PR/push; product run scheduled/manual | `tests/support/product_coverage.py`, `tests/support/warehouse_scope.py`, `tests/e2e`, source publishers and API routers |
