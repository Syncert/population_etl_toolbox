...
| Frontend | WEB-001–WEB-025 | None |
| Deployment | DEPLOY-001–DEPLOY-005 | None |
| **Total** | **235 of 235** | **0 of 235** |
...
| WEB-014 | P0 | Unit + Browser / `frontend` | Neutral-resource access for dispatch-shaped sources | A source joins the explorer through whichever access shape its capability entry declares — the source-scoped latest/timeseries pair or the neutral `/observations` resource — a segment-less source keeps its published `source_code` as its shareable key, every request filter comes from the declared `observation_filters`/route parameters so an undeclared one is neither sent nor silently dropped, `/distribution/bins` is requested only where declared, the neutral envelope's period bounds, unit, status, and dimensions map onto the explorer row shape without touching the published value, and a stratified answer is reported and left uncolored rather than collapsed to one row per geography | An undeclared filter reaching a request or a silently widened answer, a dispatch-shaped source unreachable or reached by an invented route, a suppressed value rendered as a number, a published period range narrowed to one bound, or several declared-dimension series presented as one value per geography |
