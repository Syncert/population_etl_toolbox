...
- [ ] Put the source's relation DDL under its own package: control and silver in `src/data_ingestion_toolbox/<source>/DDL/`, the publication views in `<source>/gold_<subject>/DDL/`. Reference each file from `sql/bootstrap/warehouse_manifest.json` in the `silver`, `gold` and `publisher` phases, and apply them at run time from an `ensure_*_schema` task upstream of every capture, so the DAG repairs a warehouse a step behind it rather than writing to one. A step under `sql/migrations/` carries only what a rerunnable file cannot -- a data rewrite or a constraint swap on a populated warehouse -- and never a second copy of a view body. A source whose relations exist only in a migration is refused by `tests/unit/shared/test_warehouse_manifest.py` (DB-054).
